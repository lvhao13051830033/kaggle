...
train_group1 = trainData.groupby(['Sex', 'Pclass'])  # 返回类型是Groupby类型，里边有key1*key2组
# sum()是将各组内每个特征的所有数据相加，返回也是key1*key2组，只不过每组里只有一行数据，是所有数据之和
# count()是统计各组每个特征的数据量（由于有缺失数据，所以各个特征间是不一定一样的）
...
pl.bar(x-wi, rate.female, wi*2, color='r')
pl.bar(x+wi, rate.male, wi*2, color='b')
...
trainData['Embarked'] = trainData['Embarked'].map({'C': -1, 'Q': 0, 'S': 1})
# numpy.isnan()函数的作用是返回一个Boolean数组，对应input的下标，若当前位置是nan那么就是true，否则false
# 对一个数组挑选时可以用array[True,False,True...]True就是选择
# 挑选出age为nan的，这个集合不用age这个特征，做四特征预测
train_age_nan = trainData[np.isnan(trainData['Age'])]
# 挑选出age不为空的数据，直接调用dropna(),这个方法会去掉输入数组中有nan特征的数据，挑选后的具备所有特征，所以可以用所有特征
train_all = trainData.dropna()
